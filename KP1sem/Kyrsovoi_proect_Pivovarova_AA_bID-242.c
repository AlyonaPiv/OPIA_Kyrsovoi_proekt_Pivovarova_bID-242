...
int** sort_rascheska(int** C, int s3, int k3);					// сортировка расческой по возрастанию элементов диагонали матрицы
...
int main() {
	...
	while (1) { // бесконечный цикл с возможностью выхода
		char o;

		puts("\n\t     Чтобы возможно было совершить умножение, необходимо ввести корректные данные: \n\t    количество строк второй матрицы должно совпадать с количеством столбцов первой.\n\t\t\t\t\t  Будьте внимательны!");
		...
		printf("\n\tВыберите способ инициализации массивов: \n\t\t 1 - собственноручно; \n\t\t 2 - с помощью генерации случайных чисел\n\t\t\t");
		scanf_s("%d", &spsb);
		...
		case 2:
			printf("\n\tВведите максимальное значение диапазона > 1\n\t(Например, вы вводите '10' - диапазон будет от -10 до 10):\t  ");
			scanf_s("%d", &d);
			...
			break;
		default:
			printf("Произошли технические неполадки, попробуйте ещё раз!");
			break;
		}
